...
def take_enemy_hp(cords):
    ...
    if len(lvl5_hp) > 0:
        lvl5_hp.pop()
        print(lvl5_hp)
    create_hp_bar(lvl5_hp, 40)


def create_hp_bar(hp_list, width):
    global green_hp, red_hp, boss_enemy_sprite, hp_bar_call

    if len(hp_bar_call) > 0:
        for call in hp_bar_call:
            canvas.after_cancel(call)
    ...
    if green_hp is None and red_hp is None:
        ...
        if len(cords) > 0:
            hp_x_start = cords[0] - width/2*5
            hp_y_start = cords[1] - 100
            hp_x_end_green = (cords[0] - lenhp//2*5) + (5*len(hp_list))
            hp_x_end_red = cords[0] + width/2*5
        ...
        if len(hp_list) == 0:
            canvas.delete("enemy")
        hp_after = canvas.after(50, create_hp_bar, hp_list, width)
        hp_bar_call.append(hp_after)
